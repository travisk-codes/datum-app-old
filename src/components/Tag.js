...
const HalfName = ({ name, color, onClick, classes }) => (
	<Chip
		clickable={true}
		label={name}
		onClick={onClick}
...
const HalfValue = ({ value, color, onClick, classes }) => (
	<Chip
		clickable
		label={value}
		onClick={onClick}
...
const Tag = props => {
	...
	if (name && value) {
		return (
			<NameValuePair {...props} name={name} value={value} />
		)
	} else if (name) {
		return (
			<Name
